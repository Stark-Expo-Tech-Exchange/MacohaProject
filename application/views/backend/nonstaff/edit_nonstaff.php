<?php
$nonstaff_id  = $this->db->get_where('nonstaff', array('nonstaff_code' => $param2))->row()->nonstaff_id;
$nonstaff     = $this->db->get_where('nonstaff', array('nonstaff_id' => $nonstaff_id))->result_array();
foreach ($nonstaff as $row):
    $first_designation = $this->db->get_where('designation', array('nonstaff_id' => $nonstaff_id))->row(); ?>
   <div class="row">
                    <div class="col-sm-12">
				  	<div class="panel panel-info">
                            <div class="panel-heading"> <i class="fa fa-edit"></i>&nbsp;&nbsp;<?php echo get_phrase('edit_nonstaff'); ?></div>
										<div class="panel-body table-responsive">

                    <?php echo form_open(base_url() . 'nonstaff/nonstaff/update/' . $row['nonstaff_code'],
                   ...
                   <div class="form-group">
                    ...
                    <span id="designation">
                        ...
                        <?php
                        $query = $this->db->get_where('designation', array('nonstaff_id' => $nonstaff_id));
                        if($query->num_rows() > 1) {
                            $count          = 1;
                    ...
                    <div class="form-group">
                        <button type="submit" class="btn btn-success btn-block btn-rounded btn-sm"> <i class="fa fa-plus"></i>&nbsp; <?php echo get_phrase('edit_nonstaff'); ?></button>
                    </div>
                    <?php echo form_close(); ?>
                </div>
            </div>
...
<script>
    ...
    function deleteParentElement(n, designation_id) {
        $.ajax({
            url     : '<?php echo base_url(); ?>nonstaff/delete_designation/' + designation_id,
